...
/**
 * packed knobs bit 14: Somersloop item list follows extractors (before remaining-bytes
 * recipe overrides). Same density as {@link FLAG_HAS_EXTERNAL}: n u8 + n × itemIndex.
 */
const FLAG_HAS_SLOOPED = 1 << 14;
/**
...
export type PlanSnapshot = {
  ...
  /**
   * Mode B: item ids with full Somersloop amplification (2× output).
   * Empty when absent.
   */
  sloopedItems: string[];
};
...
export type PlanHashSource = {
  ...
  recipeOverrides?: Record<string, string>;
  /** Mode B Somersloop-amplified steps (optional; omitted when empty). */
  sloopedItems?: string[];
};
...
/**
 * Normalize Somersloop-amplified item ids. Sorted for stable hashes; capped at
 * {@link MAX_SLOOPED}. Drops ids missing from the share-hash catalog.
 */
export function normalizeSloopedItems(ids: string[] | undefined | null): string[] {
  if (!ids || ids.length === 0) return [];
  const seen = new Set<string>();
  const out: string[] = [];
  for (const raw of ids) {
    ...
    seen.add(id);
    out.push(id);
    if (out.length >= MAX_SLOOPED) break;
  }
  out.sort((a, b) => a.localeCompare(b));
  return out;
}
...
export function toSnapshot(source: PlanHashSource): PlanSnapshot {
  ...
  return {
    ...
    sloopedItems: normalizeSloopedItems(source.sloopedItems),
  };
}
...
function sloopedToEncode(ids: string[]): number[] {
  const out: number[] = [];
  for (const id of normalizeSloopedItems(ids)) {
    const idx = indexOfItem(id);
    if (idx < 0) continue;
    out.push(idx);
    if (out.length >= MAX_SLOOPED) break;
...
export function encodePlanBytes(snap: PlanSnapshot): Uint8Array {
  ...
  const activeSlooped = snap.mode === "product" ? sloopedToEncode(snap.sloopedItems ?? []) : [];
  ...
  const hasSlooped = activeSlooped.length > 0;
  out.push(flags);

  out.push(clampClockPercent(snap.miner.clockPercent, DEFAULT_MINER_SETTINGS.clockPercent));

  // Computation knobs (2 bytes): centerPower 5 | topN 3 | siteSep 5 | worldConfig 1 | sloop 1
  ...
  if (hasSlooped) packed |= FLAG_HAS_SLOOPED;
  ...
  if (hasSlooped) {
    out.push(activeSlooped.length & 0xff);
    for (const idx of activeSlooped) out.push(idx & 0xff);
  }
...
export function decodePlanBytes(bytes: Uint8Array): PlanSnapshot | null {
  ...
  const hasSlooped = (packed & FLAG_HAS_SLOOPED) !== 0;
  ...
  const sloopedItems: string[] = [];
  if (hasSlooped) {
    if (i >= bytes.length) return null;
    const nSloop = bytes[i++] ?? 0;
    if (nSloop > MAX_SLOOPED) return null;
    if (i + nSloop > bytes.length) return null;
    for (let k = 0; k < nSloop; k++) {
      const id = itemAt(bytes[i++] ?? 0);
      if (id) sloopedItems.push(id);
    }
  }
...
export function encodeProductPlanHash(
  ...
  opts?: {
    ...
    recipeOverrides?: Record<string, string>;
    /** Item ids with full Somersloop amplification. */
    sloopedItems?: string[];
  },
): string {
  ...
  return encodePlanHash({
    ...
    recipeOverrides,
    sloopedItems: opts?.sloopedItems,
  });
}
