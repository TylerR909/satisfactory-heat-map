...
| `public/data/recipes/items.json` | Compact item catalog (~168) | Derived from Coffee Stain Docs via `npm run parse-docs` |
| `public/data/recipes/recipes.json` | Factory recipes (~290: defaults + alts) | Same; **not** the 10MB Docs file |
| `public/data/recipes/itemIds.json` | Append-only share-hash item index | Generated by `parse-docs`; **never renumber** |
| `public/data/recipes/recipeIds.json` | Append-only share-hash recipe index | Same |
| `public/data/recipes/recipePrimaries.json` | recipe → primary product (link builders) | Same |
| `src/data/*.json` | Same tables for the app codec import | Synced by `parse-docs` |
| `public/data/recipes/docs-meta.json` | Parse stamp / counts | Generated; safe to commit |

Share-hash catalogs must stay **append-only** (never renumber). Spec: [SHARE_HASH.md](./SHARE_HASH.md). External tools: [SHARE_HASH_VENDORING.md](./SHARE_HASH_VENDORING.md).
